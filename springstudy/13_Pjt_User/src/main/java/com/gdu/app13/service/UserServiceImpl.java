...
public class UserServiceImpl implements UserService {
	...
	@Transactional
	// insert와 delete가 동시에 진행되기 때문에 트랜잭셔널 처리
	@Override
	...
	public void restoreUser(HttpServletRequest request, HttpServletResponse response) {
		// HttpServletRequest request는 비밀번호를 받아옴		
		// 계정 복원을 원하는 사용자의 아이디
		HttpSession session = request.getSession();
	...
	public String getNaverLoginApiURL(HttpServletRequest request) {
		...
		try {
		    String clientId = "luiB6A4n2eXDEubyWuRA";//애플리케이션 클라이언트 아이디값";
	...
	public UserDTO getNaverLoginTokenNProfile(HttpServletRequest request) {
		...
		String clientId = "luiB6A4n2eXDEubyWuRA";//애플리케이션 클라이언트 아이디값";
	    String clientSecret = "E2983l6y80";//애플리케이션 클라이언트 시크릿값";
	    String code = request.getParameter("code");
	    String state = request.getParameter("state");
	    String redirectURI = null;
	    try {
	    	redirectURI = URLEncoder.encode("http://localhost:9090" + request.getContextPath() + "/user/naver/login", "UTF-8"); // 여기 오류~~
