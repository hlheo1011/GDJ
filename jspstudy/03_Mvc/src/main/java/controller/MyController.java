...
// @WebServlet({"/today.do", "/now.do"}) // 맵핑을 계속 추가해줘야하니 좋은 방법은 아님
...
public class MyController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String command = requestURI.substring(contextPath.length() + 1);	// 8째 인덱스부터 끝까지 가져오기(today.do 또는 now.do)
		...
		// 2. actionForwadr == null : response로 응답한 경우 또는 ajax 처리
		...
		// TodayService의 execute에 있는 request와 response는 같은 요청이다. 하나의 메소드.
		
		request.getRequestDispatcher("result.jsp").forward(request, response);
		// result.jsp로 forward를 보내주겠다
	}
