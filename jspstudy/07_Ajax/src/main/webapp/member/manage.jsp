...
<script>
	...
	function fn_getAllMembers(){
		$.ajax({
			...
			success: function(resData){  // resData : {"count": 3, "members": [{}, {}, {}]}
				...
				// 2. member_list 영역 초기화 (기존에 있던 list를 지움)
				$('#member_list').empty();
				// 3. resData.members : [{}, {}, {}]
				...
				$.each(resData.members, function(i, member){
					...
					tr += '<td><input type="hidden" value="'+ member.memberNo + '"><input type="button" value="조회" class="btn_detail"></td>';	// 오류뜨면 여기 수정 ★★★★
					tr += '</tr>';
			...
			$.ajax({
				...
				success: function(resData){  // resData : {"exists": true, "member": {"id": "user3", ...}}
					if(resData.exists) {
						...
						// $(':radio[name=gender]').val(M).prop('checked', true);
						// 위 주석 코드는 radio 이름이 gender인 value를 전부 M으로 바꿔주는 거다. 주의해서 사용(사용하지 말기)				
	...
	<!-- html은 원래 있던 데이터를 지우고 덮어쓰지만 .append는 기존 자료에 += 해준다. -->
	<!-- btn_detail은 JS로 준 버튼이여서 클릭 이벤트가 안먹힌다. -->
</head>
<body>
