...
public class BoardDao {
	...
	public List<Board> selectAllBoards() {
		...
		try {
			...
			rs = ps.executeQuery();		// SELECT문은 executeQuery() 사용해야 한다.
			// 첫번째 행이 rs, 그 이후로 rs.next()를 써줘야 다음 행으로 바뀐다. 그 후에는 rs.getint("board_no") = rs.getint(1) 동일 코드
	...
	public Board selectBoardByNo(int board_no) {
		...
		try {
			...
			sql = "SELECT BOARD_NO, TITLE, CONTENT, CREATE_DATE FROM BOARD WHERE BOARD_NO = ?"; // 변수는 물음표 처리
			ps = con.prepareStatement(sql);
			ps.setInt(1, board_no);	// 1번째 물음표(?)에 board_no 전달하기
			rs = ps.executeQuery(); // SELECT문은 executeQuery() 사용
			if(rs.next()) {			// 상세보기는 if문
				board = new Board();	// 조회된 결과가 있으면 Board를 만듬
				board.setBoard_no( rs.getInt(1) );	   // rs.getInt("BOARD_NO")
				board.setTitle( rs.getString(2) );	   // rs.getString("TITLE")
				board.setContent( rs.getString(3) );   // rs.getString("CONTENT")
				board.setCreate_date( rs.getDate(4) ); // rs.getDate("CREATE_DATE")
	...
	public int deleteBoard(int board_no) {
		int result = 0;
		try {
			con = dataSource.getConnection();				// 언제나 연결부터(DB)
			sql = "DELETE FROM BOARD WHERE BOARD_NO = ?";	// 쿼리문
			ps = con.prepareStatement(sql);					// 쿼리문을 실행할 수 있는 객체 ps
			ps.setInt(1, board_no);							// 변수처리 (첫번째 물음표에 board_no 넣어주세요)
			result = ps.executeUpdate();					// DELETE문은 executeUpdate() 메소드 사용. (DML은 사용하는 메소드(=executeUpdate())가 동일하다)
		} catch(Exception e) {
			e.printStackTrace();
		} finally {
			close(con, ps, null);
