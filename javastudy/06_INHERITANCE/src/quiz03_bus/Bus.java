...
public class Bus {
	...
	public Bus(int cnt) {
		...
		limit = cnt;
		for(int i = 0; i < cnt; i++) {		// 배열에 하나 하나 넣어줘야 한다.
			seats[i] = new Seat();
		}
	}
	
	// ride() 메소드
	// 2번째 매개변수는 student alba를 저장할 수 있는 Person
	public void ride(int seatNo, Person person) {
		// 존재하지 않는 시트번호 생성
		if(seatNo <= 0 || seatNo > limit) {
			return;		// if seatNo가 0보다 작거나 같을때나, seatNo가 limit 보다 크면, ride() 메소드 종료
		}
