...
-- 트리거 TRIG3 동작 확인
UPDATE EMPLOYEE SET NAME = '팔창민' WHERE EMP_NO = 1001; -- 구창민
DELETE FROM EMPLOYEE WHERE EMP_NO = 1001; -- 바꾼다음에 확인 한거여서 '팔창민' 나옴
ROLLBACK;
...
-- 목표 : 사원(EMPLOYEES) 테이블에서 삭제된 사원정보를 퇴사자(RETIRERS) 테이블에 삽입하기

-- 1) 퇴사자 테이블 만들기(EMPLOYEES 테이블과 동일한 구조, 데이터)
DROP TABLE RETIRERS;
CREATE TABLE RETIRERS
    AS (SELECT * FROM EMPLOYEES WHERE 1 = 2);

-- 2) RETIRE_ID, RETIRE_DATE(퇴사날짜) 칼럼 추가하기
ALTER TABLE RETIRERS ADD RETIRE_ID NUMBER NOT NULL;
ALTER TABLE RETIRERS ADD RETIRE_DATE DATE;

-- 3) RETIRE_ID 기본키 설정
ALTER TABLE RETIRERS ADD CONSTRAINT PK_RETIRERS PRIMARY KEY(RETIRE_ID);
