...
/* 
    ...
    6. 서브쿼리 결과에 따른 구분
       1) 단일 행 서브쿼리
           (1) 서브쿼리 결과가 1개
           (2) PK나 UNIQUE 칼럼에 동등 비교(=) 결과, 함수의 결과
           (3) 단일 행 연산자를 사용(=, !=, >, >=, <, <=)
       2) 다중 행 서브쿼리
           (1) 서브쿼리 결과가 2개 이상
           (2) FROM절, WHERE절에서 사용
...
SELECT DEPT_NO, DEPT_NAME, LOCATION
 ...
 WHERE LOCATION = (SELECT LOCATION
                   FROM DEPARTMENT
                  WHERE DEPT_NO = 1); -- EMP_NO는 PK이므로 단일 행 서브쿼리
...
SELECT EMP_NO, NAME, DEPART, GENDER, POSITION, HIRE_DATE, SALARY
  FROM EMPLOYEE
 WHERE MONTHS_BETWEEN(SYSDATE, HRIE_DATE >= (SELECT AVG(MONTHS_BETWEEN(SYSDATE, HIRE_DATE))
                                              FROM EMPLOYEE);
...
SELECT EMP_NO, NAME, DEPART, GENDER, POSITION, HIRE_DATE, SALARY
 ...
 WHERE DEPART IN (SELECT DEPT_NO
                     FROM DEPARTMENT
                    WHERE DEPT_NAME = '영업부'); -- DEPT_NAME가 PK/UNIQUE가 아니기 때문에다
...
SELECT DEPT_NO, DEPT_NAME, LOCATION
 ...
 WHERE DEPT_NO IN(SELECT DEPART
                    FROM EMPLOYEE
                   WHERE POSITION = '과장'); -- POSITION이 PK/UNIQUE가 

SELECT DEPT_NO, DEPT_NAME, D.LOCATION
  FROM DEPARTMENT D INNER JOIN EMPLOYEE E
    ON D.DEPT_NO = E.DEPART
 WHERE E.POSITION = '과장';
...
-- 아래 코드는 오류. >는 단일 행 서브쿼리에서만 사용 하다.
SELECT EMP_NO, NAME, DEPART, GENDER, POSITION, HIRE_DATE, SALARY
  FROM EMPLOYEE
...
-- 따라사 스칼라 서브쿼리와 동일한 방식의 조인은 '외부조인'임
...
/*
    ...
    1. Inline-View
    2. FORM절에서 사용하는 서브쿼리를 의미함
    3. 인라인뷰는 주로 테이블 형식임
    4. 인라인뷰에 별명을 주고 사용함
    5. 인라인뷰에서 조회한 칼럼만 메인쿼리에서 조회할 수 있음
    6. 왜 사용하는지?
       SELECT문의 실행순서를 바꿀 때 사용한다.
...
SELECT EMP_NO, NAME
  FROM EMPLOYEE
 WHERE ROWID = 'AAAFI+AABAAALDJAAC';
...
-- 2. ROWNUM은 1을 포함하지 않는 범위는 조건으로 사용할 수 없음(1이 없으면 사용 불가?)
SELECT EMP_NO, NAME
  FROM EMPLOYEE
...
SELECT B.ROW_NUM, B.EMP_NO, B.NAME, B.SALARY
  FROM (SELECT ROWNUM AS ROW_NUM, EMP_NO, NAME, SALARY
          FROM (SELECT EMP_NO, NAME, SALARY
                  FROM EMPLOYEE
...
-- 2. 테이블 복사할 때 사용
-- 3. NUT NULL 제약조건을 제외한 제약조건은 복사되지 않음
-- 4. 형식
...
SELECT CONSTRAINT_NAME FROM USER_CONSTAINTS WHERE TABLE_NAME = 'EMPLOYEE2';
...
-- 3. department 테이블의 모든 행(ROW)을 DEPARTMENT2 테이블 삽입
