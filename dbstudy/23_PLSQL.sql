...
-- 문제. EMPLOYEE_ID가 100인 회원의 FIRST_NAME, LAST_NAME, SALARY 정보를 읽어서 참조변수에 저장
DECLARE
...
-- 레코드 : 필드의 모임
-- DB에서 레코드란? 행(ROW)
-- 레코드 변수 : 여러 칼럼을 동시에 저장하는 변수 (자바로 따지면 클래스)
...
BEGIN
    ...
    ELSE
        RESULT := 'F';
    END IF;
    DBMS_OUTPUT.PUT_LINE(SCORE || '점은 ' || RESULT || '학점 입니다.');
END;
...
DECLARE
    EMP_NO EMPLOYEES.EMPLOYEE_ID%TYPE;
    SAL EMPLOYEES.SALARY%TYPE;      -- EMPLYOEES 테이블의 SALARY와 같은 타입
    TOTAL NUMBER;
    CNT NUMBER;
    AVERAGE NUMBER;
BEGIN
...
-- EMPLOYEE_ID가 50인 사원을 제외하고 연봉합계 조회하기
DECLARE
    EMP_NO EMPLOYEES.EMPLOYEE_ID%TYPE;
    SAL EMPLOYEES.SALARY%TYPE;
