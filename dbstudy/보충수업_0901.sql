...
-- 3. JOB_ID가 'IT_PROG'인 사원중에서 가장 높은 연봉(SALARY)을 받는 사원의 연봉보다
--    더 많은 연봉을 받는 사원들의 EMPLOYEE_ID, FIRST_NAME, JOB_ID, SALARY를 조회하시오.

-- 서브쿼리 : 최대연봉
-- 메인쿼리 : SELECT 칼럼 FROM 테이블 WHERE 직업='IT_PROG' AND 연봉 > (직업 'IT_PROG'의 최대연봉)
...
-- 메인쿼리 : SELECT 칼럼 FROM 테이블 WHERE (JOB_ID, DEPARTMENT_ID) IN (서브쿼리) : 사용 못함. IN은 다중 행일때 사용. 서브쿼리 보면 결과값이 1줄만 나오므로 = 연산자가 맞다.
-- IN으로 해도 (잘못된 문법이지만,) 오라클이 알아는 들음
...
-- CITY, LOCATION_ID - LOCATION_ID, DEPARTMENT_D - DEPARTMENT_ID
--       (PK)        - (FK)         (PK)           (FK)

-- 1) 조인
-- 3개의 테이블이 물려있을땐 INNER JOIN이 JOIN보다 ,가 더 편한것 같다.
...
-- 조건 : 근무중인사원수 = 최대사원수
-- HAVING절? WHERE절?                         -- 집계하는 함수는 100% HAVING 절 이다.
...
-- 최종 결과  : 인라인뷰 B에서 행 번호(RN)가 1인 행 조회
