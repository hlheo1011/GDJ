...
SELECT
     ...
     , EXTRACT(HOUR   FROM SYSTIMESTAMP) AS 시 -- UTC 기준(세계 표준시) (우리나라는 + 9:00)
     , EXTRACT(MINUTE FROM SYSTIMESTAMP) AS 분
     , EXTRACT(SECOND FROM SYSTIMESTAMP) AS 초
     , EXTRACT(TIMEZONE_HOUR FROM SYSTIMESTAMP) AS 시 -- 우선 TIMEZONE 설정이
...
--       (12시간을 숫자 0.5로 처리)
--    2) 특정 단위 후 날짜
--       (1) 1년 후   : + 12개월 후 계산, 함수 없음(함수가 필요 없기 때문에..)(365 아님)
--       (2) 1개월 후 : ADD_MONTHS 함수 사용(일정하지 않기에 함수가 존재)
--       (3) 1일 후   : + 1, 함수 없음(함수가 필요 없기 때문에..)
